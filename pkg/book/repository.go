...
func (r *repository) CreateBook(book *entities.Book) (*entities.Book, error) {
	...
	existingErr := r.Collection.FindOne(context.Background(), filter).Decode(&existingBook)
	if existingErr == nil {
		// Book already exists
		return nil, fmt.Errorf("book with title '%s' by author '%s' already exists", book.Title, book.Author)
...
func (r *repository) DeleteBook(ID string) error {
	bookID, err := bson.ObjectIDFromHex(ID)
	if err != nil {
		return err
	}
